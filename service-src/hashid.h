#ifndef snownet_hashid_h
#define snownet_hashid_h
...
hashid_init(struct hashid *hi, int max) {
	...
	hi->cap = max;
	hi->count = 0;
	hi->id = (struct hashid_node *)snownet_malloc(max * sizeof(struct hashid_node));
	for (i=0;i<max;i++) {
		hi->id[i].id = -1;
		hi->id[i].next = NULL;
	}
	hi->hash = (struct hashid_node **)snownet_malloc(hashcap * sizeof(struct hashid_node *));
