#ifndef snownet_databuffer_h
#define snownet_databuffer_h
...
static void
databuffer_read(struct databuffer *db, struct messagepool *mp, void * buffer_, int sz) {
	assert(db->size >= sz);
	db->size -= sz;
...
databuffer_push(struct databuffer *db, struct messagepool *mp, void *data, int sz) {
	...
	if (mp->freelist) {
		...
		struct messagepool_list * mpl = (struct messagepool_list *)snownet_malloc(sizeof(*mpl));
