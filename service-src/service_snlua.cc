...
init_cb(struct snlua *l, struct snownet_context *ctx, const char * args, size_t sz) {
	...
	lua_pushlstring(L, args, sz);//args 第一次调用这个回调默认参数是bootstrap,消息是snlua_init的时候发送的也应该是整个系统的第一个消息
...
launch_cb(struct snownet_context * context, void *ud, int type, int session, uint32_t source , const void * msg, size_t sz) {
	...
	snownet_callback(context, NULL, NULL);//bootstrap程序只需要处理一次就够了，所以这里关闭回调
...
snlua_init(void* l_, struct snownet_context *ctx, const char * args) {
	...
	int sz = strlen(args);//此处默认是 bootstrap 因为配置默认是 bootstrap = "snlua bootstrap"	-- The service for bootstrap
	 ...
	 * 把参数当作消息内容发给这个服务，就是 snownet.newservice(name, ...) 后面的 ...
	 * 目的是驱动服务完成初始化，后面会讲到，snownet服务是消息驱动。
	 * 调用这个函数的时候，工作线程还没有工作，消息还不会处理,
	 * 所以下面snownet_send的消息要等系统开始处理消息的时候才会进入launch_cb
	 */
