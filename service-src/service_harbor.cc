...
push_queue_msg(struct harbor_msg_queue * queue, struct harbor_msg * m) {
	...
	if (((queue->tail + 1) % queue->size) == queue->head) {
		struct harbor_msg * new_buffer = (struct harbor_msg *)snownet_malloc(queue->size * 2 * sizeof(struct harbor_msg));
		int i;
		for (i=0;i<queue->size-1;i++) {
			new_buffer[i] = queue->data[(i+queue->head) % queue->size];
		}
		snownet_free(queue->data);
...
new_queue() {
	struct harbor_msg_queue * queue = (struct harbor_msg_queue *)snownet_malloc(sizeof(*queue));
	queue->size = DEFAULT_QUEUE_SIZE;
	queue->head = 0;
	queue->tail = 0;
	queue->data = (struct harbor_msg *)snownet_malloc(DEFAULT_QUEUE_SIZE * sizeof(struct harbor_msg));
...
hash_insert(struct hashmap * hash, const char name[GLOBALNAME_LENGTH]) {
	...
	struct keyvalue * node = (struct keyvalue *)snownet_malloc(sizeof(*node));
...
hash_new() {
	struct hashmap * h = (struct hashmap *)snownet_malloc(sizeof(struct hashmap));
	memset(h,0,sizeof(*h));
	return h;
}
...
close_harbor(struct harbor *h, int id) {
	...
	if (s->fd) {
		snownet_socket_close(h->ctx, s->fd);
	}
	if (s->queue) {
...
report_harbor_down(struct harbor *h, int id) {
	...
	snownet_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, down, n);
}

void *
harbor_create(void) {
	struct harbor * h = (struct harbor *)snownet_malloc(sizeof(*h));
	memset(h,0,sizeof(*h));
	h->map = hash_new();
	return h;
}
...
void
harbor_release(void *h_) {
	struct harbor * h = (struct harbor *)h_;
	int i;
	for (i=1;i<REMOTE_MAX;i++) {
		...
		if (s->fd && s->status != STATUS_DOWN) {
			...
			// never call snownet_send during module exit, because of dead lock
		}
	}
	hash_delete(h->map);
	snownet_free(h);
...
forward_local_messsage(struct harbor *h, void *msg, int sz) {
	...
	if (snownet_send(h->ctx, header.source, destination, type | PTYPE_TAG_DONTCOPY , (int)header.session, (void *)msg, sz-HEADER_COOKIE_LENGTH) < 0) {
		if (type != PTYPE_ERROR) {
			// don't need report error when type is error
			snownet_send(h->ctx, destination, header.source , PTYPE_ERROR, (int)header.session, NULL, 0);
		}
		snownet_error(h->ctx, "Unknown destination :%x from :%x type(%d)", destination, header.source, type);
	}
}

static void
send_remote(struct snownet_context * ctx, int fd, const char * buffer, size_t sz, struct remote_message_header * cookie) {
	size_t sz_header = sz+sizeof(*cookie);
	if (sz_header > UINT32_MAX) {
		snownet_error(ctx, "remote message from :%08x to :%08x is too large.", cookie->source, cookie->destination);
		return;
	}
	uint8_t * sendbuf = (uint8_t *)snownet_malloc(sz_header+4);
	...
	snownet_socket_send(ctx, fd, sendbuf, sz_header+4);
}
...
dispatch_name_queue(struct harbor *h, struct keyvalue * node) {
	...
	struct snownet_context * context = h->ctx;
	...
	if (fd == 0) {
		if (s->status == STATUS_DOWN) {
			...
			snownet_error(context, "Drop message to %s (in harbor %d)",tmp,harbor_id);
		} else {
			if (s->queue == NULL) {
				s->queue = node->queue;
				node->queue = NULL;
			...
			if (harbor_id == (h->slave >> HANDLE_REMOTE_SHIFT)) {
				...
				while ((m = pop_queue(s->queue)) != NULL) {
					...
					snownet_send(context, m->header.source, handle , type | PTYPE_TAG_DONTCOPY, m->header.session, m->buffer, m->size);
				}
				release_queue(s->queue);
				s->queue = NULL;
			}
	...
	while ((m = pop_queue(queue)) != NULL) {
		...
		send_remote(context, fd, (const char*)m->buffer, m->size, &m->header);
		snownet_free(m->buffer);
	}
}
...
dispatch_queue(struct harbor *h, int id) {
	...
	struct harbor_msg * m;
	while ((m = pop_queue(queue)) != NULL) {
		send_remote(h->ctx, fd, (const char*)m->buffer, m->size, &m->header);
		snownet_free(m->buffer);
	}
	release_queue(queue);
	s->queue = NULL;
...
static void
push_socket_data(struct harbor *h, const struct snownet_socket_message * message) {
	assert(message->type == SNOWNET_SOCKET_TYPE_DATA);
	int fd = message->id;
	int i;
	int id = 0;
	struct slave * s = NULL;
	...
	if (s == NULL) {
		snownet_error(h->ctx, "Invalid socket fd (%d) data", fd);
		return;
	}
	...
	for (;;) {
		...
		case STATUS_HANDSHAKE: {
			...
			if (remote_id != id) {
				snownet_error(h->ctx, "Invalid shakehand id (%d) from fd = %d , harbor = %d", id, fd, remote_id);
				close_harbor(h,id);
				return;
			}
			++buffer;
		...
		case STATUS_HEADER: {
			...
			if (size < need) {
				...
				if (s->size[0] != 0) {
					snownet_error(h->ctx, "Message is too long from harbor %d", id);
					close_harbor(h,id);
					return;
				}
				s->length = s->size[1] << 16 | s->size[2] << 8 | s->size[3];
				s->read = 0;
				s->recv_buffer = (char*)snownet_malloc(s->length);
...
remote_send_handle(struct harbor *h, uint32_t source, uint32_t destination, int type, int session, const char * msg, size_t sz) {
	...
	if (harbor_id == h->id) {
		// local message
		snownet_send(context, source, destination , type | PTYPE_TAG_DONTCOPY, session, (void *)msg, sz);
		return 1;
	}
	...
	if (s->fd == 0 || s->status == STATUS_HANDSHAKE) {
		if (s->status == STATUS_DOWN) {
			...
			snownet_send(context, destination, source, PTYPE_ERROR, 0 , NULL, 0);
			snownet_error(context, "Drop message to harbor %d from %x to %x (session = %d, msgsz = %d)",harbor_id, source, destination,session,(int)sz);
		} else {
			if (s->queue == NULL) {
				s->queue = new_queue();
			}
...
remote_send_name(struct harbor *h, uint32_t source, const char name[GLOBALNAME_LENGTH], int type, int session, const char * msg, size_t sz) {
	...
	if (node->value == 0) {
		...
		snownet_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, query, strlen(query));
		return 1;
	} else {
...
handshake(struct harbor *h, int id) {
	struct slave *s = &h->s[id];
	uint8_t * handshake = (uint8_t *)snownet_malloc(1);
	handshake[0] = (uint8_t)h->id;
	snownet_socket_send(h->ctx, s->fd, handshake, 1);
}
...
harbor_command(struct harbor * h, const char * msg, size_t sz, int session, uint32_t source) {
	...
	case 'N' : {
		if (s <=0 || s>= GLOBALNAME_LENGTH) {
			snownet_error(h->ctx, "Invalid global name %s", name);
			return;
		}
	...
	case 'S' :
	case 'A' : {
		char *buffer = (char*)snownet_malloc(s+1);
		memcpy(buffer, name, s);
		buffer[s] = 0;
		int fd=0, id=0;
		sscanf(buffer, "%d %d",&fd,&id);
		if (fd == 0 || id <= 0 || id>=REMOTE_MAX) {
			snownet_error(h->ctx, "Invalid command %c %s", msg[0], buffer);
			return;
		}
		struct slave * slave = &h->s[id];
		if (slave->fd != 0) {
			snownet_error(h->ctx, "Harbor %d alreay exist", id);
			return;
		}
		slave->fd = fd;

		snownet_socket_start(h->ctx, fd);
		handshake(h, id);
		if (msg[0] == 'S') {
	...
	default:
		snownet_error(h->ctx, "Unknown command %s", msg);
		return;
	}
}
...
static int
mainloop(struct snownet_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	struct harbor * h = (struct harbor *)ud;
	switch (type) {
	case PTYPE_SOCKET: {
		const struct snownet_socket_message * message = (const struct snownet_socket_message *)msg;
		switch(message->type) {
		case SNOWNET_SOCKET_TYPE_DATA:
			push_socket_data(h, message);
			snownet_free(message->buffer);
			break;
		case SNOWNET_SOCKET_TYPE_ERROR:
		case SNOWNET_SOCKET_TYPE_CLOSE: {
			int id = harbor_id(h, message->id);
			if (id) {
				report_harbor_down(h,id);
			} else {
				snownet_error(context, "Unkown fd (%d) closed", message->id);
			}
			break;
		}
		case SNOWNET_SOCKET_TYPE_CONNECT:
			// fd forward to this service
			break;
		case SNOWNET_SOCKET_TYPE_WARNING: {
			int id = harbor_id(h, message->id);
			if (id) {
				snownet_error(context, "message havn't send to Harbor (%d) reach %d K", id, message->ud);
			}
			break;
		}
		default:
			snownet_error(context, "recv invalid socket message type %d", type);
			break;
		}
		return 0;
	}
	...
	case PTYPE_SYSTEM : {
		...
		snownet_free((void *)rmsg->message);
		return 0;
	}
	default:
		snownet_error(context, "recv invalid message from %x,  type = %d", source, type);
		if (session != 0 && type != PTYPE_ERROR) {
			snownet_send(context,0,source,PTYPE_ERROR, session, NULL, 0);
		}
		return 0;
	}
}
...
int
harbor_init(void *h_, struct snownet_context *ctx, const char * args) {
	struct harbor * h = (struct harbor *)h_;
	h->ctx = ctx;
	int harbor_id = 0;
	uint32_t slave = 0;
	...
	h->id = harbor_id;
	h->slave = slave;
	snownet_callback(ctx, h, mainloop);
	snownet_harbor_start(ctx);
