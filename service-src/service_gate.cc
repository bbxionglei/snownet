...
extern char *strsep_(char **stringp, const char *delim);

struct connection {
	int id;	// snownet_socket id
	uint32_t agent;
	uint32_t client;
	char remote_name[32];
...
void *
gate_create(void) {
	struct gate * g = (struct gate *)snownet_malloc(sizeof(*g));
	memset(g,0,sizeof(*g));
	g->listen_id = -1;
	return g;
}
...
void
gate_release(void *g_) {
	struct gate* g = (struct gate*)g_;
	int i;
	struct snownet_context *ctx = g->ctx;
	for (i=0;i<g->max_connection;i++) {
		struct connection *c = &g->conn[i];
		if (c->id >=0) {
			snownet_socket_close(ctx, c->id);
		}
	}
	if (g->listen_id >= 0) {
		snownet_socket_close(ctx, g->listen_id);
	}
...
_ctrl(struct gate * g, const void * msg, int sz) {
	...
	if (memcmp(command,"broker",i)==0) {
		_parm(tmp, sz, i);
		g->broker = snownet_queryname(ctx, command);
		return;
	}
	...
	snownet_error(ctx, "[gate] Unkown command : %s", command);
}
...
_report(struct gate * g, const char * data, ...) {
	...
	snownet_send(ctx, 0, g->watchdog, PTYPE_TEXT,  0, tmp, n);
}
...
_forward(struct gate *g, struct connection * c, int size) {
	...
	if (g->broker) {
		void * temp = snownet_malloc(size);
		databuffer_read(&c->buffer,&g->mp,temp, size);
		snownet_send(ctx, 0, g->broker, g->client_tag | PTYPE_TAG_DONTCOPY, fd, temp, size);
		return;
	}
	if (c->agent) {
		void * temp = snownet_malloc(size);
		databuffer_read(&c->buffer,&g->mp,temp, size);
		snownet_send(ctx, c->client, c->agent, g->client_tag | PTYPE_TAG_DONTCOPY, fd , temp, size);
	} else if (g->watchdog) {
		char * tmp = (char*)snownet_malloc(size + 32);
		int n = snprintf(tmp,32,"%d data ",c->id);
		databuffer_read(&c->buffer,&g->mp,tmp+n,size);
		snownet_send(ctx, 0, g->watchdog, PTYPE_TEXT | PTYPE_TAG_DONTCOPY, fd, tmp, size + n);
	}
}
	...
	for (;;) {
		...
		if (size < 0) {
			...
			if (size >= 0x1000000) {
				...
				snownet_socket_close(ctx, id);
				snownet_error(ctx, "Recv socket message > 16M");
				return;
			} else {
				_forward(g, c, size);
...
static void
dispatch_socket_message(struct gate *g, const struct snownet_socket_message * message, int sz) {
	struct snownet_context * ctx = g->ctx;
	switch(message->type) {
	case SNOWNET_SOCKET_TYPE_DATA: {
		...
		if (id>=0) {
			...
			snownet_error(ctx, "Drop unknown connection %d message", message->id);
			snownet_socket_close(ctx, message->id);
	...
	case SNOWNET_SOCKET_TYPE_CONNECT: {
		...
		int id = hashid_lookup(&g->hash, message->id);
		if (id<0) {
			snownet_error(ctx, "Close unknown connection %d", message->id);
			snownet_socket_close(ctx, message->id);
		}
		break;
	}
	...
	case SNOWNET_SOCKET_TYPE_ACCEPT:
		// report accept, then it will be get a SNOWNET_SOCKET_TYPE_CONNECT message
		assert(g->listen_id == message->id);
		if (hashid_full(&g->hash)) {
			snownet_socket_close(ctx, message->ud);
		} else {
			...
			snownet_error(ctx, "socket open: %x", c->id);
		}
		break;
	case SNOWNET_SOCKET_TYPE_WARNING:
		snownet_error(ctx, "fd (%d) send buffer (%d)K", message->id, message->ud);
		break;
	}
}

static int
_cb(struct snownet_context * ctx, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	...
	case PTYPE_CLIENT: {
		if (sz <=4 ) {
			snownet_error(ctx, "Invalid client message from %x",source);
			break;
		}
		...
		if (id>=0) {
			...
			snownet_socket_send(ctx, uid, (void*)msg, sz-4);
			// return 1 means don't free msg
			return 1;
		} else {
			snownet_error(ctx, "Invalid client id %d from %x",(int)uid,source);
			break;
		}
	}
	case PTYPE_SOCKET:
		// recv socket message from snownet_socket
		dispatch_socket_message(g, (const snownet_socket_message*)msg, (int)(sz-sizeof(struct snownet_socket_message)));
		break;
	}
	return 0;
}
...
start_listen(struct gate *g, char * listen_addr) {
	...
	if (portstr == NULL) {
		...
		if (port <= 0) {
			snownet_error(ctx, "Invalid gate address %s",listen_addr);
			return 1;
		}
	} else {
	...
	g->listen_id = snownet_socket_listen(ctx, host, port, BACKLOG);
	if (g->listen_id < 0) {
		return 1;
	}
	snownet_socket_start(ctx, g->listen_id);
	return 0;
}

int
gate_init(void *g_ , struct snownet_context * ctx, const char * parm) {
	struct gate* g = (struct gate*)g_;
	if (parm == NULL)
		return 1;
	int max = 0;
	int sz = strlen(parm) + 1;
	char *watchdog = (char*)snownet_malloc(sz);
	char *binding = (char*)snownet_malloc(sz);
	int client_tag = 0;
	char header;
	int n = sscanf(parm, "%c %s %s %d %d", &header, watchdog, binding, &client_tag, &max);
	if (n<4) {
		snownet_error(ctx, "Invalid gate parm %s",parm);
		return 1;
	}
	if (max <=0 ) {
		snownet_error(ctx, "Need max connection");
		return 1;
	}
	if (header != 'S' && header !='L') {
		snownet_error(ctx, "Invalid data header style");
		return 1;
	}
	...
	if (watchdog[0] == '!') {
		...
		g->watchdog = snownet_queryname(ctx, watchdog);
		if (g->watchdog == 0) {
			snownet_error(ctx, "Invalid watchdog %s",watchdog);
			return 1;
		}
	}
	...
	g->conn = (struct connection *)snownet_malloc(max * sizeof(struct connection));
