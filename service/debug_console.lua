...
local codecache = require "snownet.codecache"
...
local function console_main_loop(stdin, print)
	print("Welcome to snownet console")
	snownet.error(stdin, "connected")
	...
	if not ok then
		snownet.error(stdin, err)
	end
	snownet.error(stdin, "disconnected")
	socket.close(stdin)
end

snownet.start(function()
	local listen_socket = socket.listen (ip, port)
	snownet.error("Start debug console at " .. ip .. ":" .. port)
	socket.start(listen_socket , function(id, addr)
		...
		socket.start(id)
		snownet.fork(console_main_loop, id , print)
	end)
end)
...
function COMMAND.start(...)
	local ok, addr = pcall(snownet.newservice, ...)
	if ok then
		if addr then
			return { [snownet.address(addr)] = ... }
		else
			return "Exit"
		end
	else
...
function COMMAND.log(...)
	local ok, addr = pcall(snownet.call, ".launcher", "lua", "LOGLAUNCH", "snlua", ...)
	if ok then
		if addr then
			return { [snownet.address(addr)] = ... }
		else
...
function COMMAND.service()
	return snownet.call("SERVICE", "lua", "LIST")
end

local function adjust_address(address)
	local prefix = address:sub(1,1)
	if prefix == '.' then
		return assert(snownet.localname(address), "Not a valid name")
	elseif prefix ~= ':' then
		address = assert(tonumber("0x" .. address), "Need an address") | (snownet.harbor(snownet.self()) << 24)
	end
	return address
end

function COMMAND.list()
	return snownet.call(".launcher", "lua", "LIST")
end

function COMMAND.stat()
	return snownet.call(".launcher", "lua", "STAT")
end

function COMMAND.mem()
	return snownet.call(".launcher", "lua", "MEM")
end

function COMMAND.kill(address)
	return snownet.call(".launcher", "lua", "KILL", address)
end

function COMMAND.gc()
	return snownet.call(".launcher", "lua", "GC")
end

function COMMAND.exit(address)
	snownet.send(adjust_address(address), "debug", "EXIT")
end
...
function COMMAND.inject(address, filename, ...)
	...
	f:close()
	local ok, output = snownet.call(address, "debug", "RUN", source, filename, ...)
	if ok == false then
		error(output)
	end
	return output
...
function COMMAND.task(address)
	address = adjust_address(address)
	return snownet.call(address,"debug","TASK")
end

function COMMAND.info(address, ...)
	address = adjust_address(address)
	return snownet.call(address,"debug","INFO", ...)
end

function COMMANDX.debug(cmd)
	local address = adjust_address(cmd[2])
	local agent = snownet.newservice "debug_agent"
	local stop
	...
	local function forward_cmd()
		repeat
			...
			snownet.call(agent, "lua", "ping")	-- detect agent alive, if agent exit, raise error
			local cmdline = socket.readline(cmd.fd, "\n")
			cmdline = cmdline and cmdline:gsub("(.*)\r$", "%1")
			if not cmdline then
				snownet.send(agent, "lua", "cmd", "cont")
				break
			end
			snownet.send(agent, "lua", "cmd", cmdline)
		until stop or cmdline == "cont"
	end
	snownet.fork(function()
		pcall(forward_cmd)
		if not stop then	-- block at snownet.call "start"
			term_co = nil
		else
			snownet.wakeup(term_co)
		end
	end)
	local ok, err = snownet.call(agent, "lua", "start", address, cmd.fd)
	stop = true
	if term_co then
		-- wait for fork coroutine exit.
		snownet.wait(term_co)
	end
...
function COMMAND.logon(address)
	address = adjust_address(address)
	core.command("LOGON", snownet.address(address))
end

function COMMAND.logoff(address)
	address = adjust_address(address)
	core.command("LOGOFF", snownet.address(address))
end

function COMMAND.signal(address, sig)
	address = snownet.address(adjust_address(address))
	if sig then
...
function COMMAND.ping(address)
	address = adjust_address(address)
	local ti = snownet.now()
	snownet.call(address, "debug", "PING")
	ti = snownet.now() - ti
	return tostring(ti)
end
...
function COMMAND.trace(address, proto, flag)
	...
	snownet.call(address, "debug", "TRACELOG", proto, flag)
end
...
function COMMANDX.call(cmd)
	...
	local rets = table.pack(snownet.call(address, "lua", table.unpack(args, 2, args.n)))
	return rets
end
...
local function convert_stat(info)
	...
	info.address = snownet.address(info.address)
