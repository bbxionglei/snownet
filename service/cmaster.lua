...
local function dispatch_slave(fd)
	...
	elseif t == 'Q' then
		...
		snownet.error("Invalid slave message type " .. t)
	end
end

local function monitor_slave(slave_id, slave_address)
	local fd = slave_node[slave_id].fd
	snownet.error(string.format("Harbor %d (fd=%d) report %s", slave_id, fd, slave_address))
	while pcall(dispatch_slave, fd) do end
	snownet.error("slave " ..slave_id .. " is down")
...
snownet.start(function()
	local master_addr = snownet.getenv "standalone"
	snownet.error("master listen socket " .. tostring(master_addr))
	local fd = socket.listen(master_addr)
	socket.start(fd , function(id, addr)
		snownet.error("connect from " .. addr .. " " .. id)
		socket.start(id)
		local ok, slave, slave_addr = pcall(handshake, id)
		if ok then
			snownet.fork(monitor_slave, slave, slave_addr)
		else
			snownet.error(string.format("disconnect fd = %d, error = %s", id, slave))
			socket.close(id)
		end
	end)
end)
