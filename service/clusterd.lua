...
local sc = require "snownet.socketchannel"
local socket = require "snownet.socket"
local cluster = require "snownet.cluster.core"

local config_name = snownet.getenv "cluster"
local node_address = {}
local node_session = {}
local command = {}
local config = {}
...
local function open_channel(t, key)
	...
	if address == nil and not config.nowaiting then
		...
		ct.namequery = co
		snownet.error("Waiting for cluster node [".. key.."]")
		snownet.wait(co)
...
local function loadconfig(tmp)
	...
	for name,address in pairs(tmp) do
		if name:sub(1,2) == "__" then
			name = name:sub(3)
			config[name] = address
			snownet.error(string.format("Config %s = %s", name, address))
		else
			...
			if ct and ct.namequery and not config.nowaiting then
				snownet.error(string.format("Cluster node [%s] resloved : %s", name, address))
				snownet.wakeup(ct.namequery)
			end
		end
	end
...
function command.listen(source, addr, port)
	local gate = snownet.newservice("gate")
	if port == nil then
		local address = assert(node_address[addr], addr .. " is down")
		addr, port = string.match(address, "([^:]+):(.*)$")
	end
	snownet.call(gate, "lua", "open", { address = addr, port = port })
	snownet.ret(snownet.pack(nil))
end
...
local function send_request(source, node, addr, msg, sz)
	...
	local tracetag = snownet.tracetag()
	if tracetag then
		if tracetag:sub(1,1) ~= "(" then
			-- add nodename
			local newtag = string.format("(%s-%s-%d)%s", nodename, node, session, tracetag)
			snownet.tracelog(tracetag, string.format("session %s", newtag))
			tracetag = newtag
		end
		snownet.tracelog(tracetag, string.format("cluster %s", node))
...
function command.req(...)
	...
	if ok then
		if type(msg) == "table" then
			snownet.ret(cluster.concat(msg))
		else
			snownet.ret(msg)
		end
	else
		snownet.error(msg)
		snownet.response()(false)
	end
end
...
function command.proxy(source, node, name)
	...
	if proxy[fullname] == nil then
		proxy[fullname] = snownet.newservice("clusterproxy", node, name)
	end
	snownet.ret(snownet.pack(proxy[fullname]))
end
...
local function clearnamecache()
	for fd, service in pairs(cluster_agent) do
		if type(service) == "number" then
			snownet.send(service, "lua", "namechange")
		end
	end
end
...
function command.register(source, name, addr)
	...
	snownet.ret(nil)
	snownet.error(string.format("Register [%s] :%08x", name, addr))
end

function command.queryname(source, name)
	snownet.ret(snownet.pack(register_name[name]))
end

function command.socket(source, subcmd, fd, msg)
	if subcmd == "open" then
		snownet.error(string.format("socket accept from %s", msg))
		-- new cluster agent
		cluster_agent[fd] = false
		local agent = snownet.newservice("clusteragent", snownet.self(), source, fd)
		local closed = cluster_agent[fd]
		cluster_agent[fd] = agent
		if closed then
			snownet.send(agent, "lua", "exit")
		...
		if subcmd == "close" or subcmd == "error" then
			...
			snownet.error(string.format("socket %s %d %s", subcmd, fd, msg or ""))
		end
	end
end
...
snownet.start(function()
	loadconfig()
	snownet.dispatch("lua", function(session , source, cmd, ...)
