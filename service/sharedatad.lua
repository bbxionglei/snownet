...
local sharedata = require "snownet.sharedata.corelib"
local table = table
local cache = require "snownet.codecache"
...
local function collectobj()
	while true do
		snownet.sleep(60*100)	-- sleep 1min
...
function CMD.new(name, t, ...)
	...
	elseif dt == "string" then
		...
		local _, ret = assert(snownet.pcall(f, ...))
...
function CMD.monitor(name, obj)
	...
	table.insert(v.watch, snownet.response())

	return NORET
end

snownet.start(function()
	snownet.fork(collectobj)
