...
local loaderpath = snownet.getenv"snax_loader"
...
local function return_f(f, ...)
	return snownet.ret(snownet.pack(f(...)))
end
...
snownet.start(function()
	...
	local function dispatcher( session , source , id, ...)
		...
		if method[2] == "system" then
			...
			if command == "hotfix" then
				...
				snownet.ret(snownet.pack(hotfix(func, ...)))
			elseif command == "profile" then
				snownet.ret(snownet.pack(profile_table))
	...
	snownet.dispatch("snax", dispatcher)

	-- set lua dispatcher
	function snax.enablecluster()
		snownet.dispatch("lua", dispatcher)
	end
end)
