...
local mc = require "snownet.multicast.core"
local datacenter = require "snownet.datacenter"

local harbor_id = snownet.harbor(snownet.self())
...
function command.DEL(source, c)
	...
	if node ~= harbor_id then
		snownet.send(node_address[node], "lua", "DEL", c)
		return NORET
	end
	...
	if remote then
		for node in pairs(remote) do
			snownet.send(node_address[node], "lua", "DELR", c)
		end
	end
	return NORET
end
...
local function remote_publish(node, channel, source, ...)
	snownet.redirect(node_address[node], source, "multicast", channel, ...)
end

-- publish a message, for local node, use the message pointer (call mc.bind to add the reference)
-- for remote node, call remote_publish. (call mc.unpack and snownet.tostring to convert message pointer to string)
local function publish(c , source, pack, size)
	...
	if remote then
		...
		local msg = snownet.tostring(msg,sz)
		for node in pairs(remote) do
	...
	local msg = snownet.tostring(pack, size)	-- copy (pack,size) to a string
	...
	for k in pairs(group) do
		...
		snownet.redirect(k, source, "multicast", c , msg)
	end
end

snownet.register_protocol {
	name = "multicast",
	id = snownet.PTYPE_MULTICAST,
...
function command.PUB(source, c, pack, size)
	assert(snownet.harbor(source) == harbor_id)
	local node = c % 256
	if node ~= harbor_id then
		-- remote publish
...
function command.SUBR(source, c)
	local node = snownet.harbor(source)
...
function command.SUB(source, c)
	...
	if node ~= harbor_id then
		...
		if channel[c] == nil then
			if snownet.call(node_address[node], "lua", "SUBR", c) then
				return
			end
			if channel[c] == nil then
				-- double check, because snownet.call whould yield, other SUB may occur.
				channel[c] = {}
				channel_n[c] = 0
			end
		end
	...
	if group[source] then
		...
		if channel_n[c] == 0 then
			...
			if node ~= harbor_id then
				...
				channel[c] = nil
				channel_n[c] = nil
				snownet.send(node_address[node], "lua", "USUBR", c)
			end
		end
	end
	return NORET
...
snownet.start(function()
	snownet.dispatch("lua", function(_,source, cmd, ...)
	...
	local self = snownet.self()
	local id = snownet.harbor(self)
