...
local ignoreret = snownet.ignoreret
...
local register_name_mt = { __index =
	function(self, name)
		...
		if waitco then
			...
			local addr = snownet.call(clusterd, "lua", "queryname", name:sub(2))	-- name must be '@xxxx'
			if addr then
				self[name] = addr
			end
...
local function dispatch_request(_,_,addr, session, msg, sz, padding, is_push)
	ignoreret()	-- session is fd, don't call snownet.ret
	if session == nil then
		-- trace
		tracetag = addr
		return
	...
	local ok, response
	if addr == 0 then
		local name = snownet.unpack(msg, sz)
		snownet.trash(msg, sz)
		local addr = register_name["@" .. name]
		if addr then
			ok = true
			msg, sz = snownet.pack(addr)
		else
			ok = false
			msg = "name not found"
		end
		...
		if addr then
			if is_push then
				snownet.rawsend(addr, "lua", msg, sz)
				return	-- no response
			else
				if tracetag then
					ok , msg, sz = pcall(snownet.tracecall, tracetag, addr, "lua", msg, sz)
					tracetag = nil
				else
					ok , msg, sz = pcall(snownet.rawcall, addr, "lua", msg, sz)
				end
			end
		else
			ok = false
...
snownet.start(function()
	...
	snownet.dispatch("lua", function(_,source, cmd, ...)
		...
		elseif cmd == "namechange" then
			...
			snownet.error(string.format("Invalid command %s from %s", cmd, snownet.address(source)))
		end
	end)
end)
