...
buffer_reserve(struct buffer *b, int sz) {
	...
	if (b->ptr == b->buffer) {
		b->ptr = (uint8_t*)snownet_malloc(b->cap);
		memcpy(b->ptr, b->buffer, b->size);
	} else {
		b->ptr = (uint8_t*)snownet_realloc(b->ptr, b->cap);
	}
}
...
//		int32_t length; // total message size, including this
	...
	const reply_st* reply = (const reply_st *)data;
...
LUAMOD_API int
luaopen_snownet_mongo_driver(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] ={
		{ "query", op_query },
		{ "reply", op_reply },
