...
// msg should alloc by snownet_malloc 
static struct stm_copy *
stm_newcopy(void * msg, int32_t sz) {
	struct stm_copy * copy = (struct stm_copy *)snownet_malloc(sizeof(*copy));
	copy->reference = 1;
	copy->sz = sz;
	copy->msg = msg;
...
stm_new(void * msg, int32_t sz) {
	struct stm_object * obj = (struct stm_object *)snownet_malloc(sizeof(*obj));
...
LUAMOD_API int
luaopen_snownet_stm(lua_State *L) {
	luaL_checkversion(L);
