...
pack(lua_State *L, void *data, size_t size) {
	struct mc_package * pack = (struct mc_package *)snownet_malloc(sizeof(struct mc_package));
	pack->reference = 0;
	pack->size = (uint32_t)size;
	pack->data = data;
	struct mc_package ** ret = (struct mc_package **)snownet_malloc(sizeof(*ret));
	*ret = pack;
...
LUAMOD_API int
luaopen_snownet_multicast_core(lua_State *L) {
	luaL_Reg l[] = {
		{ "pack", mc_packlocal },
