...
lfreepool(lua_State *L) {
	struct buffer_node * pool = (struct buffer_node *) (struct buffer_node *)lua_touserdata(L, 1);
	int sz = lua_rawlen(L,1) / sizeof(*pool);
	int i;
	for (i=0;i<sz;i++) {
		struct buffer_node *node = &pool[i];
		if (node->msg) {
			snownet_free(node->msg);
			node->msg = NULL;
		}
	}
	return 0;
...
lnewpool(lua_State *L, int sz) {
	struct buffer_node * pool = (struct buffer_node *) lua_newuserdata(L, sizeof(struct buffer_node) * sz);
	int i;
	for (i=0;i<sz;i++) {
		pool[i].msg = NULL;
		pool[i].sz = 0;
...
lnewbuffer(lua_State *L) {
	struct socket_buffer * sb = (struct socket_buffer *) lua_newuserdata(L, sizeof(*sb));
	sb->size = 0;
	sb->offset = 0;
	sb->head = NULL;
	sb->tail = NULL;
...
lpushbuffer(lua_State *L) {
	struct socket_buffer *sb = (struct socket_buffer *)lua_touserdata(L,1);
	if (sb == NULL) {
	...
	struct buffer_node * free_node = (struct buffer_node*)lua_touserdata(L,-1);	// sb poolt msg size free_node
	lua_pop(L,1);
	if (free_node == NULL) {
		...
		lnewpool(L, size);	
		free_node = (struct buffer_node *)lua_touserdata(L,-1);
...
return_free_node(lua_State *L, int pool, struct socket_buffer *sb) {
	...
	free_node->next = (struct buffer_node *)lua_touserdata(L,-1);
	lua_pop(L,1);
	snownet_free(free_node->msg);
	free_node->msg = NULL;
...
static int
lunpack(lua_State *L) {
	struct snownet_socket_message *message = (struct snownet_socket_message*)lua_touserdata(L,1);
	...
	if (message->type == SNOWNET_SOCKET_TYPE_UDP) {
		int addrsz = 0;
		const char * addrstring = snownet_socket_udp_address(message, &addrsz);
		if (addrstring) {
...
lconnect(lua_State *L) {
	...
	struct snownet_context * ctx = (struct snownet_context*)lua_touserdata(L, lua_upvalueindex(1));
	int id = snownet_socket_connect(ctx, host, port);
...
lshutdown(lua_State *L) {
	int id = luaL_checkinteger(L,1);
	struct snownet_context * ctx = (struct snownet_context*)lua_touserdata(L, lua_upvalueindex(1));
	snownet_socket_shutdown(ctx, id);
	return 0;
}
...
llisten(lua_State *L) {
	...
	struct snownet_context * ctx = (struct snownet_context*)lua_touserdata(L, lua_upvalueindex(1));
	int id = snownet_socket_listen(ctx, host,port,backlog);
	if (id < 0) {
...
lsend(lua_State *L) {
	...
	int err = snownet_socket_send(ctx, id, buffer, sz);
...
lsendlow(lua_State *L) {
	...
	int sz = 0;
	void *buffer = get_buffer(L, 2, &sz);
	int err = snownet_socket_send_lowpriority(ctx, id, buffer, sz);
	lua_pushboolean(L, !err);
	return 1;
}
...
lbind(lua_State *L) {
	...
	int id = snownet_socket_bind(ctx,fd);
	lua_pushinteger(L,id);
	return 1;
}
...
ludp(lua_State *L) {
	...
	snownet_free(tmp);
	int id = snownet_socket_udp(ctx, host, port);
	if (id < 0) {
...
ludp_connect(lua_State *L) {
	...
	snownet_free(tmp);
	if (snownet_socket_udp_connect(ctx, id, host, port)) {
...
ludp_send(lua_State *L) {
	...
	int err = snownet_socket_udp_send(ctx, id, address, buffer, sz);
...
linfo(lua_State *L) {
	lua_newtable(L);
	struct socket_info * si = snownet_socket_info();
...
LUAMOD_API int
luaopen_snownet_socketdriver(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "buffer", lnewbuffer },
		{ "push", lpushbuffer },
