...
lpackresponse(lua_State *L) {
	...
	// and the msg/sz is return by snownet.rawcall , so don't free(msg)
...
lconcat(lua_State *L) {
	...
	lua_pop(L,1);
	char * buff = (char *)snownet_malloc(sz);
	int idx = 2;
	int offset = 0;
...
LUAMOD_API int
luaopen_snownet_cluster_core(lua_State *L) {
	luaL_Reg l[] = {
