...
channel_read(struct channel *c, double timeout) {
	...
	if (c->head == NULL) {
		...
		std::this_thread::sleep_for(std::chrono::microseconds(ti));
		return NULL;
	}
	ret = c->head;
	c->head = ret->next;
...
channel_write(struct channel *c, const char * s, size_t sz) {
	struct command * cmd = (struct command *)malloc(sizeof(*cmd)+ sz);
	cmd->sz = sz;
	cmd->next = NULL;
	memcpy(cmd+1, s, sz);
	SPIN_LOCK(c)
...
static int
lread(lua_State *L) {
	struct channel_box *cb = (struct channel_box *)luaL_checkudata(L,1, METANAME);
...
lrelease(lua_State *L) {
	struct channel_box *cb = (struct channel_box *)lua_touserdata(L, 1);
	if (cb) {
...
new_channel(lua_State *L, struct channel *c) {
	...
	struct channel_box * cb = (struct channel_box *)lua_newuserdata(L, sizeof(*cb));
	cb->c = c;
...
LUAMOD_API int
luaopen_snownet_debugchannel(lua_State *L) {
	luaL_Reg l[] = {
