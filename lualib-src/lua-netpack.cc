...
get_queue(lua_State *L) {
	struct queue *q = (struct queue *)lua_touserdata(L,1);
	if (q == NULL) {
		q = (struct queue *)lua_newuserdata(L, sizeof(struct queue));
		q->cap = QUEUESIZE;
		q->head = 0;
		q->tail = 0;
		int i;
...
expand_queue(lua_State *L, struct queue *q) {
	struct queue *nq = (struct queue *)lua_newuserdata(L, sizeof(struct queue) + q->cap * sizeof(struct netpack));
...
save_uncomplete(lua_State *L, int fd) {
	struct queue *q = get_queue(L);
	int h = hash_fd(fd);
	struct uncomplete * uc = (struct uncomplete *)snownet_malloc(sizeof(struct uncomplete));
...
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	...
	if (size < pack_size) {
		...
		uc->pack.buffer = snownet_malloc(pack_size);
...
filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	...
	if (uc) {
		...
		if (size < need) {
			int8_t* bf = (int8_t*)uc->pack.buffer;
			memcpy(bf + uc->read, buffer, size);
			uc->read += size;
			int h = hash_fd(fd);
			uc->next = q->hash[h];
			q->hash[h] = uc;
		...
		if (size == pack_size) {
			...
			lua_pushvalue(L, lua_upvalueindex(TYPE_DATA));
			lua_pushinteger(L, fd);
			void * result = snownet_malloc(pack_size);
...
static int
lfilter(lua_State *L) {
	struct snownet_socket_message *message = (struct snownet_socket_message *)lua_touserdata(L,2);
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = (uint8_t *)snownet_malloc(len + 2);
...
LUAMOD_API int
luaopen_snownet_netpack(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "pop", lpop },
		{ "pack", lpack },
