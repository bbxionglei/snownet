...
create_proxy(lua_State *L, const void *data, int index) {
	const struct table * t = gettable((const document*)data, index);
	if (t == NULL) {
	...
	struct proxy * p = (struct proxy*)lua_newuserdata(L, sizeof(struct proxy));
...
update_cache(lua_State *L, const void *data, const void * newdata) {
	...
	while (lua_next(L, t) != 0) {
		...
		if (lua_rawget(L, pt) == LUA_TUSERDATA) {
			// pointer, table, proxy
			struct proxy * p = (struct proxy*)lua_touserdata(L, -1);
			if (p->data == data) {
				// update to newdata
				p->data = (const char*)newdata;
				const struct table * newt = gettable((struct document*)newdata, p->index);
				lua_pop(L, 1);
				// pointer, table
				clear_table(L);
				lua_pushvalue(L, lua_upvalueindex(1));
...
LUAMOD_API int
luaopen_snownet_datasheet_core(lua_State *L) {
	luaL_checkversion(L);
	luaL_Reg l[] = {
		{ "new", lnew },
		{ "update", lupdate },
