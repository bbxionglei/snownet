...
local function assert_socket(service, v, fd)
	if v then
		...
		snownet.error(string.format("%s failed: socket (fd = %d) closed", service, fd))
		error(socket_error)
	end
end
...
local function launch_slave(auth_handler)
	...
	local function ret_pack(ok, err, ...)
		if ok then
			return snownet.pack(err, ...)
		else
			if err == socket_error then
				return snownet.pack(nil, "socket error")
			else
				return snownet.pack(false, err)
			end
		end
	end

	local function auth_fd(fd, addr)
		snownet.error(string.format("connect from %s (fd = %d)", addr, fd))
	...
	snownet.dispatch("lua", function(_,_,...)
		local ok, msg, len = pcall(auth_fd, ...)
		if ok then
			snownet.ret(msg,len)
		else
			snownet.ret(snownet.pack(false, msg))
		end
	end)
end
...
local function accept(conf, s, fd, addr)
	-- call slave auth
	local ok, server, uid, secret = snownet.call(s, "lua",  fd, addr)
...
local function launch_master(conf)
	...
	snownet.dispatch("lua", function(_,source,command, ...)
		snownet.ret(snownet.pack(conf.command_handler(command, ...)))
	end)

	for i=1,instance do
		table.insert(slave, snownet.newservice(SERVICE_NAME))
	end

	snownet.error(string.format("login server listen at : %s %d", host, port))
	...
	socket.start(id , function(fd, addr)
		...
		if not ok then
			if err ~= socket_error then
				snownet.error(string.format("invalid client (fd = %d) error = %s", fd, err))
			end
		end
...
local function login(conf)
	...
	snownet.start(function()
		local loginmaster = snownet.localname(name)
		if loginmaster then
