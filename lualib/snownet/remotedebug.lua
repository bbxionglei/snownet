...
local injectrun = require "snownet.injectcode"
local table = table
local debug = debug
...
local function remove_hook(dispatcher)
	...
	snownet.error "Leave debug mode"
end
...
local ctx_snownet = debug.getinfo(snownet.start,"S").short_src	-- skip when enter this source file
...
function linehook(mode, line)
	...
	if mode ~= "line" then
		ctx.needupdate = true
		if mode ~= "return" then
			if ctx.next_mode or debug.getinfo(2,"S").short_src == ctx_snownet then
				ctx.level = 1
		...
		if ctx.needupdate then
			...
			if ctx.filename == ctx_term then
				ctx_active[co] = nil
				sethook()
				change_prompt(string.format(":%08x>", snownet.self()))
				return
			end
		end
...
local function watch_proto(protoname, cond)
	local proto = assert(replace_upvalue(snownet.register_protocol, "proto"), "Can't find proto table")
...
function dbgcmd.s(co)
	local ctx = ctx_active[co]
	ctx.next_mode = false
	snownet_suspend(co, coroutine.resume(co))
end
	...
	if func then
		local function idle()
			if raw_dispatcher then
			    snownet.timeout(10,idle)	-- idle every 0.1s
			end
		end
		snownet.timeout(0, idle)
	end
	return func
end
...
function M.start(import, fd, handle)
	...
	snownet_suspend = import.suspend
	assert(raw_dispatcher == nil, "Already in debug mode")
	snownet.error "Enter debug mode"
	local channel = debugchannel.connect(handle)
	raw_dispatcher = hook_dispatch(dispatcher, snownet.response(), fd, channel)
end
