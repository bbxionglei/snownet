...
function multicast.new(conf)
	...
	if self.channel == nil then
		self.channel = snownet.call(multicastd, "lua", "NEW")
	end
	self.__pack = conf.pack or snownet.pack
	self.__unpack = conf.unpack or snownet.unpack
...
function chan:delete()
	...
	snownet.send(multicastd, "lua", "DEL", c)
	self.channel = nil
...
function chan:publish(...)
	local c = assert(self.channel)
	snownet.call(multicastd, "lua", "PUB", c, mc.pack(self.__pack(...)))
end
...
function chan:subscribe()
	...
	snownet.call(multicastd, "lua", "SUB", c)
...
function chan:unsubscribe()
	...
	local c = assert(self.channel)
	snownet.send(multicastd, "lua", "USUB", c)
	self.__subscribe = nil
	dispatch[c] = nil
end
...
local function init()
	multicastd = snownet.uniqueservice "multicastd"
