local driver = require "snownet.socketdriver"
local snownet = require "snownet"
local snownet_core = require "snownet.core"
local assert = assert
...
local function suspend(s)
	...
	if s.closing then
		snownet.wakeup(s.closing)
	end
end

-- read snownet_socket.h for these macro
-- SNOWNET_SOCKET_TYPE_DATA = 1
socket_message[1] = function(id, size, data)
	local s = socket_pool[id]
	if s == nil then
		snownet.error("socket: drop package from " .. id)
	...
	if rrt == "number" then
		...
		if s.buffer_limit and sz > s.buffer_limit then
			snownet.error(string.format("socket buffer overflow: fd=%d size=%d", id , sz))
...
-- SNOWNET_SOCKET_TYPE_ERROR = 5
socket_message[5] = function(id, _, err)
	local s = socket_pool[id]
	if s == nil then
		snownet.error("socket: error on unknown", id, err)
		return
	end
	if s.connected then
		snownet.error("socket: error on", id, err)
...
-- SNOWNET_SOCKET_TYPE_UDP = 6
socket_message[6] = function(id, size, data, address)
	local s = socket_pool[id]
	if s == nil or s.callback == nil then
		snownet.error("socket: drop udp package from " .. id)
		driver.drop(data, size)
		return
	end
	local str = snownet.tostring(data, size)
	snownet_core.trash(data, size)
...
local function default_warning(id, size)
	...
	snownet.error(string.format("WARNING: %d K bytes need to send out (fd = %d)", size, id))
end

-- SNOWNET_SOCKET_TYPE_WARNING
...
snownet.register_protocol {
	name = "socket",
	id = snownet.PTYPE_SOCKET,	-- PTYPE_SOCKET = 6
...
function socket.shutdown(id)
	...
	if s then
		driver.clear(s.buffer,buffer_pool)
		-- the framework would send SNOWNET_SOCKET_TYPE_CLOSE , need close(id) later
		driver.shutdown(id)
	end
end
...
function socket.close(id)
	...
	if s.connected then
		...
		-- because snownet.wait never return in __gc, so driver.clear may not be called
		if s.co then
