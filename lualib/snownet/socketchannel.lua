	...
	while self.__sock do
		...
		if ok and session then
			...
			if co then
				...
				self.__thread[session] = nil
				snownet.error("socket: unknown session :", session)
			end
		else
...
local function pop_response(self)
	while true do
		...
		snownet.wait(self.__wait_response)
	end
end
...
local function push_response(self, response, co)
	if self.__response then
		...
		if self.__wait_response then
			snownet.wakeup(self.__wait_response)
...
local function connect_backup(self)
	if self.__backup then
		for _, addr in ipairs(self.__backup) do
			...
			snownet.error("socket: connect to backup host", host, port)
...
local function connect_once(self)
			...
			if id == self.__sock[1] then
				if size == 0 then
					...
					if not self.__overload then
						...
						snownet.error(string.format("WARNING: %d K bytes need to send out (fd = %d %s:%s)", size, id, self.__host, self.__port))
					end
				end
			end
		end

		snownet.fork(overload_trigger, fd, 0)
	...
	self.__dispatch_thread = snownet.fork(function()
	...
	if self.__auth then
		...
		if not ok then
			...
			if message ~= socket_error then
				...
				snownet.error("socket: auth failed", message)
			end
		end
		self.__authcoroutine = false
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				snownet.error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		else
			snownet.error("socket: connect", err)
		end
		if t > 1000 then
			snownet.error("socket: try to reconnect", self.__host, self.__port)
			snownet.sleep(t)
			t = 0
		else
			snownet.sleep(t)
...
local function check_connection(self)
	if self.__sock then
		if socket.disconnected(self.__sock[1]) then
			-- closed by peer
			snownet.error("socket: disconnect detected ", self.__host, self.__port)
			close_channel_socket(self)
			return
		end
...
local function block_connect(self, once)
	...
	if r == nil then
		snownet.error(string.format("Connect to %s:%d failed (%s)", self.__host, self.__port, err))
		error(socket_error)
	else
		return r
	end
