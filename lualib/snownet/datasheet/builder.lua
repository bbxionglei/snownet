...
local dump = require "snownet.datasheet.dump"
...
local function monitor(pointer)
	snownet.fork(function()
		snownet.call(address, "lua", "collect", pointer)
...
function builder.new(name, v)
	...
	snownet.call(address, "lua", "update", name, pointer)
...
function builder.update(name, v)
	...
	snownet.send(address, "lua", "release", lp)
	dataset[name] = diff
	monitor(pointer)
end
...
function datasheet.query(source, name)
	...
	local h = handles[handle]
	h.ref = h.ref + 1
	snownet.ret(snownet.pack(handle))
end
...
function datasheet.monitor(source, handle)
	...
	if t.handle ~= handle then	-- already changes
		snownet.ret(snownet.pack(t.handle))
	else
		assert(not t.monitor[source])
		t.monitor[source]=snownet.response()
	end
end
