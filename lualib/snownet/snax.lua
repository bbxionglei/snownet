...
local snownet_send = snownet.send
local snownet_call = snownet.call
...
local function gen_req(type, handle)
	return setmetatable({} , {
		__index = function( t, k )
			...
			return function(...)
				return snownet_call(handle, "snax", id, ...)
			end
		end })
end
...
function snax.rawnewservice(name, ...)
	...
	local handle = snownet.newservice("snaxd", name)
	assert(handle_cache[handle] == nil)
	if t.system.init then
		snownet.call(handle, "snax", t.system.init, ...)
	end
	return handle
end
...
function snax.uniqueservice(name, ...)
	local handle = assert(snownet.call(".service", "lua", "LAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.globalservice(name, ...)
	local handle = assert(snownet.call(".service", "lua", "GLAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.queryservice(name)
	local handle = assert(snownet.call(".service", "lua", "QUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.queryglobal(name)
	local handle = assert(snownet.call(".service", "lua", "GQUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.kill(obj, ...)
	local t = snax.interface(obj.type)
	snownet_call(obj.handle, "snax", t.system.exit, ...)
end

function snax.self()
	return snax.bind(snownet.self(), SERVICE_NAME)
end
...
function snax.hotfix(obj, source, ...)
	local t = snax.interface(obj.type)
	return test_result(snownet_call(obj.handle, "snax", t.system.hotfix, source, ...))
end

function snax.printf(fmt, ...)
	snownet.error(string.format(fmt, ...))
end

function snax.profile_info(obj)
	local t = snax.interface(obj.type)
	return snownet_call(obj.handle, "snax", t.system.profile)
end
