...
local socketchannel	= require "snownet.socketchannel"
local snownet = require "snownet"
local driver = require "snownet.mongo.driver"
		...
		if rs_data.ok == 1 then
			...
			if rs_data.ismaster	then
				...
				if rs_data.primary then
					...
					snownet.error("WARNING: NO PRIMARY RETURN " .. rs_data.me)
					...
					snownet.error("INFO: TRY TO CONNECT " .. mongoc.__pickserver[1])
...
function auth_method:auth_scram_sha1(username,password)
	...
	if not string.sub(rnonce, 1, 12) == nonce then
		snownet.error("Server returned an invalid nonce.")
		return false
	end
	...
	if parsed_t['v'] ~= server_sig then
		snownet.error("Server returned an invalid signature.")
		return false
	end
	if not r.done then
		...
		if not r.done then
			snownet.error("SASL conversation failed to complete.")
			return false
		end
	end
	return true
