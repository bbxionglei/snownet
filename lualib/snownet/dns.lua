...
local function resolve(content)
	if #content < DNS_HEADER_LEN then
		-- drop
		snownet.error("Recv an invalid package when dns query")
		return
	end
	...
	if answers_ipv4 then
		CACHE[QTYPE.A][question.name] = { answers = answers_ipv4, ttl = snownet.now() + ttl * 100 }
	end

	if answers_ipv6 then
		CACHE[QTYPE.AAAA][question.name] = { answers = answers_ipv6, ttl = snownet.now() + ttl * 100 }
	end
	...
	if question.name ~= resp.name then
		snownet.error("Recv an invalid name when dns query")
	end
...
local function connect_server()
	...
	dns_server.fd = fd
	snownet.error(string.format("Udp server open %s:%s (%d)", dns_server.address, dns_server.port, fd))
end

local DNS_SERVER_RETIRE = 60 * 100
local function touch_server()
	dns_server.retire = snownet.now()
	...
	local function check_alive()
		if snownet.now() > dns_server.retire + DNS_SERVER_RETIRE then
			local fd = dns_server.fd
			if fd then
				dns_server.fd = nil
				socket.close(fd)
				snownet.error(string.format("Udp server close %s:%s (%d)", dns_server.address, dns_server.port, fd))
			end
		else
			snownet.timeout( 2 * DNS_SERVER_RETIRE, check_alive)
		end
	end
...
local function lookup_cache(name, qtype, ignorettl)
	...
	if result then
		if ignorettl or (result.ttl > snownet.now()) then
...
local function suspend(tid, name, qtype)
	...
	snownet.fork(function()
		snownet.sleep(TIMEOUT)
		local req = request_pool[tid]
		if req then
			-- cancel tid
			snownet.error(string.format("DNS query %s timeout", name))
			request_pool[tid] = nil
			snownet.wakeup(req.co)
		end
	end)
	snownet.wait(req.co)
	request_pool[tid] = nil
