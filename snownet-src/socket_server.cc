...
struct socket_st {
	...
	volatile std::atomic<int32_t> sending;
	int fd;
	int id;
	uint8_t protocol;
	std::atomic<uint8_t> type;
	std::atomic<int32_t> udpconnecting;
	int64_t warn_size;
	union {
		int size;
		uint8_t udp_address[UDP_ADDRESS_SIZE];
...
int xsetsockopt(int fd,int level,int optname,void *optval,int optlen) {
#ifndef _WIN32
...
int xgetsockopt(int fd,int level,int optname, void *optval,void *optlen){
#ifndef _WIN32
	return getsockopt(fd, level, optname, &optval, (socklen_t*)optlen);
#else
	return getsockopt(fd, level, optname, (char *)&optval, (int*)optlen);
#endif
}
struct request_open {
	int id;
...
#define MALLOC snownet_malloc
#define FREE snownet_free
...
socket_keepalive(int fd) {
	int keepalive = 1;
	xsetsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepalive, sizeof(keepalive));
}
...
socket_server_release(struct socket_server *ss) {
	...
	xclose(ss->sendctrl_fd);
	xclose(ss->recvctrl_fd);
...
send_list_tcp(struct socket_server *ss, struct socket_st *s, struct wb_list *list, struct socket_lock *l, struct socket_message *result) {
	while (list->head) {
		...
		for (;;) {
#ifndef _WIN32
			size_t sz = write(s->fd, tmp->ptr, tmp->sz);
#else
...
setopt_socket(struct socket_server *ss, struct request_setopt *request) {
	...
	int v = request->value;
	xsetsockopt(s->fd, IPPROTO_TCP, request->what, &v, sizeof(v));
}
...
report_connect(struct socket_server *ss, struct socket_st *s, struct socket_lock *l, struct socket_message *result) {
	...
	int code = 0;
	code = xgetsockopt(s->fd, SOL_SOCKET, SO_ERROR, &error, &len);
	if (code < 0 || error) {  
		force_close(ss,s,l, result);
		if (code >= 0)
	...
	if (can_direct_write(s,id) && socket_trylock(&l)) {
		...
		if (can_direct_write(s,id)) {
			...
			if (s->protocol == PROTOCOL_TCP) {
...
#else
				n = recv(s->fd, (char*)so.buffer, so.sz, 0);
#endif
			} else {
				union sockaddr_all sa;
				...
				n = sendto(s->fd, (const char*)so.buffer, so.sz, 0, &sa.s, sasz);
			}
			if (n<0) {
...
do_bind(const char *host, int port, int protocol, int *family) {
	...
	*family = ai_list->ai_family;
	...
	if (xsetsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse, sizeof(int))==-1) {
		goto _failed;
	}
...
socket_info_create(struct socket_info *last) {
	struct socket_info *si = (struct socket_info *)snownet_malloc(sizeof(*si));
...
class WSAInitClass {
public:
	static WSAInitClass* instance()
	{
		static WSAInitClass g_ptWAS;
		return &g_ptWAS;
	}
protected:
	WSAInitClass()
...
WSAInitClass* g_stInit = WSAInitClass::instance();
#endif