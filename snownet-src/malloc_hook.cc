...
// for snownet_lalloc use
#define raw_realloc realloc
#define raw_free free

void 
memory_info_dump(void) {
	snownet_error(NULL, "No jemalloc");
}

size_t 
mallctl_int64(const char* name, size_t* newval) {
	snownet_error(NULL, "No jemalloc : mallctl_int64 %s.", name);
	return 0;
}

int 
mallctl_opt(const char* name, int* newval) {
	snownet_error(NULL, "No jemalloc : mallctl_opt %s.", name);
	return 0;
}
...
dump_c_mem() {
	int i;
	size_t total = 0;
	snownet_error(NULL, "dump all service mem:");
	for (i = 0; i < SLOT_SIZE; i++) {
		struct mem_data* data = &mem_stats[i];
		if (data->handle != 0 && data->allocated != 0) {
			total += data->allocated;
			snownet_error(NULL, ":%08x -> %zdkb %db", data->handle, data->allocated >> 10, (int)(data->allocated % 1024));
		}
	}
	snownet_error(NULL, "+total: %zdkb", total >> 10);
}

char *
snownet_strdup(const char *str) {
	size_t sz = strlen(str);
	char * ret = (char*)snownet_malloc(sz+1);
	memcpy(ret, str, sz+1);
	return ret;
}

void * 
snownet_lalloc(void *ptr, size_t osize, size_t nsize) {
	if (nsize == 0) {
		raw_free(ptr);
		return NULL;
	} else {
...
malloc_current_memory(void) {
	uint32_t handle = snownet_current_handle();
	int i;
	for (i = 0; i < SLOT_SIZE; i++) {
		struct mem_data* data = &mem_stats[i];
...
void
snownet_debug_memory(const char *info) {
	// for debug use
	uint32_t handle = snownet_current_handle();
